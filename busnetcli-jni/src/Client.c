...
jint JNICALL Java_com_lsxy_app_area_cti_busnetcli_Client_initiateLibrary(
	...
	cls_client = cls;
	// Get Head Class
	cls_head = (*env)->FindClass(env, "com/lsxy/app/area/cti/busnetcli/Head");
	if (!cls_head) {
		(*env)->ThrowNew(env, cls,
				"Can not find the class \"com/lsxy/app/area/cti/busnetcli/Head\"");
		return 0;
	}
	meth_head = (*env)->GetMethodID(env, cls_head, "<init>", "(BBBBBBBBB)V");
	if (!meth_head) {
		(*env)->ThrowNew(env, cls,
				"Can not find the constructor of class \"com/lsxy/app/area/cti/busnetcli/Head\"");
		return 0;
	}
	// Init
	...
	meth_client_recvdata = (*env)->GetStaticMethodID(env, cls, "callbackData",
			"(IBLcom/lsxy/app/area/cti/busnetcli/Head;[B)V");
...
jint JNICALL Java_com_lsxy_app_area_cti_busnetcli_Client_createConnect(
	...
	int result = SmartBusNetCli_CreateConnect((unsigned char) local_clientid,
			(int) local_clienttype, pc_masterip, (unsigned short) master_port,
			pc_slaverip, (unsigned short) slaver_port, pc_author_username,
...
jint JNICALL Java_com_lsxy_app_area_cti_busnetcli_Client_launchFlow(
	...
	const char* pc_value_list = (*env)->GetStringUTFChars(env, value_list,
			NULL);
